...
Excel 파일(colored_tags.xlsx)의 bbox 좌표를 읽어
...
DEFAULT_TYPE_STYLES: Dict[str, Tuple[Tuple[int, int, int], Tuple[int, int, int], float]] = {
    ...
    "line_no":      ((180, 180, 0), (255, 255, 0),  0.30),
...
class MarkupWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # pdf_name으로 그룹핑
            pdf_groups = df.groupby("pdf_name")
...
class TypeSettingWidget(QFrame):
    ...
    def __init__(
        ...
        self.checkbox.setMinimumWidth(120)
...
class PDFMarkupGUI(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle("PDF 마크업 도구 (폴더 기반)")
        ...
        type_configs = [
            ...
            ("line_no", "Line Number", DEFAULT_TYPE_STYLES["line_no"]),
